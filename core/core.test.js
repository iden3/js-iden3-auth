import { Core, hashBytes, hexToBytes } from './core.js';
import { Id } from './id.js';

test('test id parsers', () => {
    const typ0Hex = Uint8Array.from([0, 0]);
    const genesis032bytes = hashBytes('genesistest');
    ...
    const typ1 = new Uint8Array(hexToBytes('0001'));
    const genesis132bytes = hashBytes('genesistest');
    ...
    const id0FromBytes = Core.idFromBytes(id0.bytes());
    expect(id0.bytes()).toEqual(id0FromBytes.bytes());
    expect(id0.string()).toEqual(id0FromBytes.string());
    expect('114vgnnCupQMX4wqUBjg5kUya3zMXfPmKc9HNH4TSE').toEqual(id0FromBytes.string());

    const id1FromBytes = Core.idFromBytes(id1.bytes());
    expect(id1.bytes()).toEqual(id1FromBytes.bytes());
    expect(id1.string()).toEqual(id1FromBytes.string());
    expect('1GYjyJKqdDyzo927FqJkAdLWB64kV2NVAjaQFHbAf').toEqual(id1FromBytes.string());


    const id0FromString = Core.idFromString(id0.string());
...
test('test id from big int', () => {
    const id = Core.idFromString('11AVZrKNJVqDJoyKrdyaAgEynyBEjksV5z2NjZoPxf');
    ...
    const got = Core.idFromInt(idBigInt);
